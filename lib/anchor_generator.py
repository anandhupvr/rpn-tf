...
class AnchorGenerator:
    ...
    def generate_pyramid_anchors(self, im_size):
        '''
        ...
        # feature_shape = [pad_shape[0]//16, pad_shape[1]//16]
        ...
        img_shapes = 1
        valid_flags = [
            self._generate_valid_flags(anchors, pad_shape)
            for i in range(img_shapes)
        ]
        valid_flags = tf.stack(valid_flags, axis=0)
        # valid_flags = self._generate_valid_flags(anchors, im_size)
    ...
    def _generate_level_anchors(self, level, feature_shape):
        ...
        feature_stride = self.feature_strides
