...
from lib.utils import compute_overlaps, trim_zeros
...
class AnchorTarget:
    ...
    def build_targets(self, anchors, valid_flags, gt_boxes, gt_class_ids):
        ...
        num_imgs = gt_class_ids # namely, batchsz , 1
        for i in range(num_imgs):
            target_match, target_delta = self._build_single_target(
                anchors, valid_flags[i], gt_boxes[i])
    ...
    def _build_single_target(self, anchors, valid_flags, gt_boxes):
        ...
        '''
        gt_boxes, _ = trim_zeros(gt_boxes, "trim_0") # remove padded zero boxes, [new_N, 4]
        ...
        target_deltas = transform.bbox2delta(
